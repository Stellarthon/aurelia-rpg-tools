...
| Remove seed DATA files from the public tree (findings **R1**, **B4**) | *this pass* | `campaign_content.json` + `.seed.sql` `git rm`'d + gitignored; README updated. Verified the committed **js is already de-baked clean** (no populated secret values) — the seed was the *only* cleartext leak. `classification-report.md` (structural, no secrets) kept. **Secrets remain in git history — see held item below.** |

### ⏸ Held — needs your decision or in-browser testing (not done autonomously)
| Item | Why held |
|------|----------|
| **R1 (remainder) — purge secrets from git history** (High) | Private-repo is ruled out (free GitHub Pages needs a public repo). Seed files are now gone from HEAD (done above), but they remain in past commits, which GitHub still serves on a public repo. Fully removing them needs a **history rewrite + force-push** (`git filter-repo` / BFG) — destructive & outward-facing (rewrites every commit SHA; disrupts clones/forks/open PRs), so it's your call to greenlight. Note: the content has been public ~1 month, so already-cloned/indexed copies can't be recalled — the purge limits *future* casual discovery, not past exposure. |
