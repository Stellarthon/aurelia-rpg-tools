...
**Status:** Plan approved for implementation; nothing here is built yet.
...
**Verified against:** the current 22-module `js/` split, `sw.js`
`orion-shell-v12`. All `js/NN:line` anchors checked 2026-07-07.
...
- The two windows talk over a **`BroadcastChannel('aurelia-table-display')`**
  — same-origin, same-machine, zero latency, works offline. There is
  currently no BroadcastChannel/postMessage usage anywhere in `js/`, so the
...
Note: today the app parses no boot query params — the only `location.search`
touch is the token-strip `history.replaceState` at `js/55:278` — so
`?display=1` introduces the pattern; keep it to this one flag.
...
2. **No shared-state writes.** The display window shares `localStorage` with
   ...
   checkbox restore, and make `supaStorage.set` a no-op.
3. **No polling.** The display does not call `startPolling()` (`js/55:956`)
   or the alert/combat pollers (`js/30:38`). The BroadcastChannel is its only
   input. (The access gate passes automatically: `aurelia_access` is already
   set in the shared `localStorage`, `js/55:163`.)
4. **Read-only surface.** `display-mode` body class hides all chrome (top
   bar, panels, toggles) via CSS; pointer events limited to the fullscreen
   affordance.
...
| referee → display | `scene` | `{view, systemId?, bodyId?, camera?}` | full state sync (boot / resync) |
| referee → display | `view` | `{name:'galaxy'\|'system'\|'body'\|'station', systemId?, bodyId?}` | switch view |
...
| referee → display | `ping` | `{nx, ny}` | pulse marker at normalized (0–1) coords |
...
One new module: **`js/93-display.js`** (the 93 slot is free; must load after
...
(`js/10:2699`), `enterSystem` (`js/10:2721`), `goSystemOverview`
(`js/30:703`), `goSystem` (`js/30:1689`), `enterStation` (`js/30:1675`),
`goBack` (`js/30:1722`), and the body-view path (`js/30:1061` via
`selectBody`).

**The one place wrapping can't reach:** the galaxy camera lives inside the HX
engine's closure (`let view={x:0,y:0,scale:1}` at `js/10:1713`, applied by
`applyTransform()` at `js/10:1755`). Two small edits inside `js/10` are
required:
...
2. Export `HX.getCamera()` / `HX.setCamera({x,y,scale})` (setter assigns and
   ...
   the referee can snapshot for `scene` sync.

Camera broadcasts are rAF-throttled (≤1 msg/frame) in the referee half.

---

## 3. Phase 1 — Table Display Mode  ·  M effort
...
2. Gate changes in `js/55-auth-gating.js`: `isReferee()` returns false under
   `DISPLAY_MODE` (`:204`); skip `checkIdentity()` prompt (`:1190`); do not
   start pollers; `supaStorage.set` no-op guard in `js/50`.
3. HX camera hooks in `js/10-galaxy.js` (§2.5 — the only edits outside the
   new module).
4. New `js/93-display.js`: channel, protocol v1 (`hello`, `scene`, `view`,
   `camera`, `blank`), function wrapping, dispatch, Follow/Hold state.
5. Display chrome: `display-mode` class on `<body>`, CSS in `css/app.css`
   hiding all chrome; click-anywhere-to-fullscreen overlay shown until
   fullscreen is entered (fullscreen requires a user gesture); subtle
   "reconnecting" state if no `scene` arrives within 2 s of `hello`.
6. Referee control cluster (in the tools/settings area, `js/60`, plus a
   keyboard shortcut registered alongside the existing ones in `js/98`):
   **Open display** (`window.open(location.pathname + '?display=1', 'aurelia-display')`),
   **Follow/Hold** toggle, **Blank** toggle, connection dot (lit on `hello`,
   MVP: no heartbeat).
...
## 4. Phase 2 — Scene push: handout scenes + pings  ·  S effort
...
1. **"→ Table" button** per handout row in `renderHandoutsPanel()`
   (`js/85:1782`), broadcasting `handout` with the `handoutUrlFor(...)` URL
   (`js/50:143`). A second button appears in the referee's own lightbox
   (`openHandout`, `js/85:1773`).
2. **Display scene element**: a dedicated full-screen `<figure>` in display
   mode (letterboxed, black background, name caption togglable) — reuses the
   lightbox styling, not the lightbox element.
3. **Ping**: referee clicks anywhere in their lightbox / map → normalized
   coords → `ping` message → 2 s pulse animation on the TV at the same
   relative position. No persistence, no tokens, no grid — deliberately.
4. **Visibility guard**: pushing a handout whose `visibleTo` (item shape at
   `js/85:1692`) is not `'all'` prompts "This handout is private to X — show
   on the shared table display anyway?". The TV is physically public; the
   confirm keeps the referee's secrecy model honest.
5. `handout-close` wired to the existing close affordance (`js/85:1781`
   pattern) and to the Blank control.
...
- Ground-combat rehearsal: referee sketches/photographs a location, uploads
  via the existing `onHandoutFile` flow (`js/85:1745`), pushes to TV, pings
  positions — no new combat UI anywhere.

---

## 5. Phase 3 — Scene ambience beats  ·  S/M effort
...
`<audio id="ambience-player">` element — the app currently has **no**
`<audio>` element anywhere; the only sound is the combat WebAudio synth
(`js/80:1759-1763`), which stays untouched — or (b) an external deep link
...
2. **Scenes strip**: a compact referee panel (one row of named buttons +
   ...
   CRUD. Lives with the misc tools (`js/92`) or as a small `js/93` sibling
   section — decide at build time by size.
3. Beat firing: fade out current → fade in new; if `handoutId`/`view` set,
   also emit the Phase 1/2 display messages (a beat = audio + picture in one
   click).
4. Session-planner link (Full, not MVP): a beat picker per planned scene in
   the planner detail view (`openSessionPlanner`, `js/97:102`), so prep and
   at-table firing meet.

**MVP:** panel + URL beats + fades + stop. **Full:** planner links, external
deep-link beats, per-beat display cut.
...
1. Add the `<script>` tag in dependency order in `index.html`
   (`index.html:1093-1114`) — `js/93-display.js` goes after `92`, before
   `96`; it must stay after `10`/`30`/`85` (it wraps their globals) and
   before `98` (boot).
2. Add the file to `sw.js` `SHELL` (`sw.js:9-21`) **and bump `CACHE`**
   (`sw.js:8`, currently `orion-shell-v12` → `v13` at Phase 1, bump again
   each released phase).
3. `node tools/build-local.mjs` — the css/js count check (`:39-41`) derives
   counts dynamically but warns on mismatch; confirm no warning.
4. `node tools/strip-secrets.mjs --check` → exit 0 (verified passing today;
   none of these phases bake new campaign literals, so no new REDACT
   classifications should be needed).
5. `node tools/verify-split.mjs` untouched by these phases (no edits to the
   original monolith mapping).

**Known limitation to document in-app:** `BroadcastChannel` requires an
...
## 8. Phase 5 (optional) — Whisper notes  ·  S effort
...
| Popup blocker eats the display window | Open is always from a direct click on "Open table display" (user gesture), never programmatic. |
...
|---|---|---|
| Month 1 | **Phase 1** Table Display Mode (M) | Acceptance §3; one real session with the TV |
| Month 2 | **Phase 2** Scene push (S) + **Phase 3** ambience MVP (S) | Acceptance §4/§5; run a ground-combat scene and a scene-beat cut at the table |
